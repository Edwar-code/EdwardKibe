...
// Change 1: Import useEffect to fetch data when the component opens
...
export default function BuyMeACoffee() {
  ...
  // Change 2: Add new state for managing the progress bar
  const [currentRaised, setCurrentRaised] = useState(0);
  const [isLoadingProgress, setIsLoadingProgress] = useState(true);
  const goalAmount = 210000;

  // Change 3: Add a useEffect to fetch the donation total when the modal opens
  useEffect(() => {
    if (isOpen) {
  ...
  }, [isOpen]); // This code re-runs every time you open the modal
  ...
  const onSuccess = (reference: any) => {
    ...
    // Change 4: Optimistically update the progress bar for a great user experience
  ...
  // Change 5: Calculate the percentage for the progress bar display
